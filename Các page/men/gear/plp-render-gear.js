/**
 * accessories-render.js
 * Render danh sách sản phẩm (PLP) từ mảng dữ liệu ACCESSORY_PRODUCTS (accessories-products.js).
 */
...
(function () {
    ...
    function createProductCard(product) {
        ...
        const badgeHtml = product.badge ? `<p class="product-badge">${product.badge}</p>` : "";
    ...
    `;
        ...
        card.addEventListener("click", () => {
            window.location.href = `./product-detail-gear.html?id=${product.id}`;
        });
    ...
    function init() {
        if (!grid) return;
        renderList(ACCESSORY_PRODUCTS);

        const sortMenu = document.getElementById("sort-menu");
        if (sortMenu) {
            sortMenu.querySelectorAll(".plp-sort-option").forEach((option) => {
                option.addEventListener("click", () => {
                    const currentFiltered = window.PLP._currentList || ACCESSORY_PRODUCTS;
